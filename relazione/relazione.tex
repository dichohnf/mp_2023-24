...
\begin{document}	
	...
	L'elaborato è stato implementano e testato attraverso l'utilizzo dell'IDE Eclipse, versione Linux 2024-06 (4.32.0) (fino alla versione Java \texttt{java.verision=21.0.4}), sul sistema Kubuntu, kernel \texttt{6.8.0-38-generic} e utilizzando per il progetto la versione 11 di Java (\texttt{java-11-openjdk-amd64}).
	...
	Per testare il progetto sono state implementate all'interno della cartella sorgente \textit{tests} le classi fittizie \texttt{jds.MockStreamChannel} e \texttt{jds.videostream.MockFrameCompositor}, servizi non implementati del sistema, e la classe \texttt{DisplaySetterToTestMulfunctions} che occorre nelle classi di test della gerarchia \texttt{MulfunctionChecker} per settare direttamente alcuni campi di display al di fuori dal pacchetto in cui risiede la classe \texttt{StandardDisplay}.
	...
	\lstinputlisting[firstline=43, lastline=59, caption={Test per la classe \texttt{jds.display.mulfunction.UnexpectedlyChangedResolutionCheckerTest}.}, label=lst:test-changed-resolution]{../tests/jds/display/mulfunction/UnexpectedlyChangedResolutionCheckerTest.java}
	...
	\lstinputlisting[firstline=60, lastline=68, caption={Test del metodo \textit{getData()} per la classe \texttt{jds.display.videostream.ComposedFrame}.}, label=lst:test-composedframe-getdata]{../tests/jds/videostream/ComposedFrameTest.java}
